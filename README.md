...
> Herramienta de escritorio de código abierto para **medir anillos de árboles**, **co-datar series** y **controlar la calidad del fechado**, desde la imagen escaneada hasta la cronología.
...
- [Características](#características)
  ...
  - [Localizador de quiebres de datación](#-localizador-de-quiebres-de-datación)
  - [Control de calidad tipo COFECHA](#-control-de-calidad-tipo-cofecha)
  - [Cronologías](#-cronologías)
  - [Unir y ordenar archivos](#-unir-y-ordenar-archivos)
...
DPI (Dendro Pixel Interface) es una aplicación de escritorio multiplataforma que cubre el flujo completo del trabajo dendrocronológico: medir, co-datar, verificar y construir la cronología, sin cambiar de programa.

Combina un **visor interactivo de imágenes** con herramientas de edición y un **motor de detección automática** configurable por especie; cálculo de **intensidad de madera** (Blue, Red, Green Intensity); **co-datación visual** por correlación en ventana deslizante; un **localizador del año donde se rompe la datación**; **control de calidad compatible con COFECHA**; y construcción de **cronologías** con estadísticos de señal común.

La pieza que distingue a DPI es el localizador de quiebres. COFECHA señala qué segmentos de 50 años son sospechosos y con qué desfase calzarían mejor; DPI responde la pregunta siguiente: **cuántos anillos hay que agregar o quitar, en qué dirección, y por cuáles años empezar a mirar**.
...
### 🔎 Localizador de quiebres de datación

Responde **en qué año se rompe el fechado**, no solo dónde baja la correlación.

Trata el año del quiebre como el parámetro a estimar: para cada año candidato y cada desfase evalúa la partición completa de la serie y elige la que maximiza la correlación conjunta. Con sumas acumuladas cada partición se evalúa en tiempo constante, así que probar miles de hipótesis toma milisegundos.

**Entrega tres cosas por separado**, porque cada una tiene su propia confiabilidad:

- **Cuántos anillos** sobran o faltan, y con qué verbo: *insertar 3 anillos*, no «+3».
- **En qué rango de años**, con un intervalo calculado en escala z de Fisher.
- **Qué años revisar primero**, ordenados por lo estrecho que es cada uno en la cronología: un anillo ausente aparece donde el árbol casi no formó leño.

**Contraste contra el azar.** La búsqueda prueba miles de configuraciones, así que siempre encuentra algo. DPI mide cuánto gana esa misma búsqueda en las otras series de la colección, que están fechadas, y solo se pronuncia si lo supera. Sobre cuatro especies: 0–11 % de falsos positivos y 78–100 % de detección.

**Informe en prosa y perfil del quiebre.** El informe explica el diagnóstico en lenguaje corriente. El perfil grafica la correlación de la serie completa suponiendo el error en cada año: un pico angosto significa que el año está determinado; una meseta, que elegir uno sería falsa precisión.

**Confiabilidad medida**, con errores introducidos en años conocidos sobre cuatro especies:
...
El último caso es un límite del método: si los errores se compensan, el desplazamiento neto es cero y ninguna partición lo revela. En esos casos el programa se calla en vez de inventar.

---

### ✅ Control de calidad tipo COFECHA

Reproduce el informe de siete partes de COFECHA (Holmes 1983), implementado de forma independiente a partir de su artículo, del manual de la Dendrochronology Program Library y de la comparación con salidas reales.

**Fidelidad medida** contra corridas reales, sobre tres colecciones:

| conjunto | series | DPI | COFECHA |
|---|---|---|---|
| Ciprés | 69 | 0,5795 | 0,5793 |
| Araucaria | 169 | 0,5722 | 0,5674 |
| Lenga | 36 | 0,4565 | 0,4466 |

El error medio por serie es 0,018. Con umbrales de decisión de 0,30 y 0,40 no hay ninguna discrepancia con COFECHA en las 238 series de las dos colecciones grandes.

**Rendimiento:** 1600 series en 8,8 segundos. El análisis corre en segundo plano, con barra de progreso y botón de cancelar.

---

### 📈 Cronologías

Construcción de cronologías `raw`, `standard` y `residual`, con cinco métodos de estandarización y tres de agregación.

**Estadísticos de señal común**, con una distinción que suele pasarse por alto:

- **Rbar entre árboles.** Dos radios del mismo árbol comparten el individuo, no solo el sitio. Contarlos como réplica independiente infla el Rbar, la EPS y la SSS. DPI separa las series por árbol y calcula el Rbar efectivo.
- **SSS** además de EPS. La EPS compara contra una población infinita de árboles; la SSS, contra la colección que realmente tienes. Para decidir desde qué año usar la cronología, la segunda es la pregunta correcta.

**Aviso de curvas de ajuste negativas.** Cuando el ajuste cruza cero o se acerca demasiado, el índice se dispara. DPI lo detecta, descarta la curva completa —como ARSTAN y dplR— y abre un diálogo con las cinco curvas superpuestas para elegir otro método, por grupo o por serie.

---

### 🔗 Unir y ordenar archivos

Combina varios archivos Tucson en uno, con manejo de códigos duplicados.

Incluye **ordenamiento por código dendrocronológico**, que lee `QLH107A` como tres cosas —sitio, número de árbol y radio— en vez de como una cadena. Ordenar alfabéticamente daría COS1, COS10, COS2. Sirve también para ordenar un archivo ya unido.
...
En Linux, para que DPI aparezca en el menú de aplicaciones y su ícono salga en
el dock —también en Wayland—, ejecuta una sola vez:
...
### 6 bis. Guardar la estimación de médula

El cálculo **no se guarda solo**. Tras marcar los puntos, pulsa `ENTER` o el
botón **📐 Ver resultado** para abrir el cuadro con los métodos, el rango de
incertidumbre y el botón de aplicar. Si cambias de herramienta con un cálculo
pendiente, el programa pregunta antes de descartarlo.

Con **📄 Guardar estimación** se exporta a un Excel aparte, con una hoja de
resumen y otra con un anillo estimado por fila. **No entra en el `.rwl`**: ese
archivo lleva anchos medidos, y meter anillos estimados contaminaría el
cofechado.
...
### 9. Co-datar y verificar

Con la serie y una cronología de referencia cargadas:

1. **Ver cofechado actual** y la correlación móvil, para saber si hay problema
   y de qué tipo: una caída que se recupera es error de datación; una que no
   se recupera es otra cosa.
2. **Buscar anillos faltantes o sobrantes.** Empieza con el máximo en 1 o 2.
3. **Leer el informe antes que la tabla.** La tabla da años; el informe dice
   cuántos anillos, en qué rango y con qué respaldo.
4. **Mirar el perfil.** Si es una meseta, no te cases con el primer año.
5. **Probar, no aplicar.** Ver el efecto en la correlación móvil.
6. **Confirmar en el skeleton** antes de aplicar. La decisión final es visual.
7. **Volver al control de calidad** para verificar que el segmento se limpió.

### 9 bis. Co-datación visual (detalle)
...
```
.
├── run_dpi.py                  # Lanzador con pantalla de inicio
├── main_anillos.py             # Ventana principal
├── modulo_medicion.py          # Visor interactivo, exportador, medición
├── modulo_codatacion.py        # Co-datación, skeleton, cronologías, spline
├── modulo_quiebres.py          # Localizador del año del quiebre
├── modulo_cofecha.py           # Motor del control de calidad
├── modulo_cofecha_nucleo.py    # Álgebra vectorizada del motor
├── modulo_cofecha_salida.py    # Formato del informe de siete partes
├── modulo_cofecha_ui.py        # Pestaña de control de calidad
├── modulo_transformacion.py    # Transformaciones comunes de correlación
├── modulo_unir.py              # Unión y ordenamiento de archivos
├── modulo_autodeteccion.py     # Detección automática de límites
├── modulo_intensidad.py        # Blue/Red/Green Intensity
├── modulo_pith.py              # Estimación de médula faltante
├── constantes.py               # Configuración global centralizada
├── estilos.py                  # Hojas de estilo Qt
├── instalar_lanzador_linux.py  # Lanzador de escritorio para Linux
├── species_profiles.json       # Perfiles de especie
├── requirements.txt            # Dependencias
├── CHANGELOG.md                # Registro de cambios
├── CITATION.cff                # Cómo citar este programa
...
Si usas DPI en un trabajo publicado, por favor cítalo. El archivo
...
DPI reimplementa procedimientos publicados. **No deriva del código fuente de
ningún otro programa**: el control de calidad se construyó a partir del
artículo de Holmes (1983), del manual de la Dendrochronology Program Library y
de la comparación con salidas reales. Si tu trabajo se apoya en esos métodos,
corresponde citar también las fuentes originales:

- Holmes, R.L. (1983). Computer-assisted quality control in tree-ring dating
  and measurement. *Tree-Ring Bulletin* 43: 69-78.
- Cook, E.R. y Peters, K. (1981). The smoothing spline: a new approach to
...
El paquete **dplR** se usó como referencia publicada para verificar
equivalencias numéricas. DPI no incorpora su código.
